package com.unifisweproject.hotelsupplymanagement.controller.supplier;
...
import com.unifisweproject.hotelsupplymanagement.window.supplier.SupplierAddWindow;
import com.unifisweproject.hotelsupplymanagement.window.supplier.SupplierDisplayWindow;
import com.unifisweproject.hotelsupplymanagement.window.supplier.SupplierSearchWindow;
import com.unifisweproject.hotelsupplymanagement.view.supplier.SupplierManagementView;
...
public class SupplierManagementController implements DataManagementController {
    ...
    private final SupplierManagementModel model;
    private ArrayList<Supplier> results = new ArrayList<>();
    private SupplierManagementView view;
    ...
    private SupplierManagementController() {// Costruttore privato per evitare la creazione di nuove istanze (SingleTon)
        ...
        model = SupplierManagementModel.getInstance();
    ...
    public void initializeRows()  {
        ...
        if (!mainMenuWindow.getIsNotFirstTimeLoad().get(2)) {
            try {
                model.loadFromDB();
                mainMenuWindow.getIsNotFirstTimeLoad().set(2, true);
            }
            catch (SQLException e) {
                System.err.println("Errore durante il riempimento della tabella");
            }
        }
        ObservableList<Supplier> supplierRows = FXCollections.observableArrayList(model.getSupplierList());
        view.setRows(supplierRows);
    ...
    public void modifyRow(ActionEvent event, Supplier displayedSupplier) {
        ...
        try {
            ...
            model.modify(displayedSupplier);
    ...
    public void displayView(ActionEvent event) {            // Metodo per visualizzare la finestra di gestione degli articoli
        ...
        try {
            view = new SupplierManagementView();                                                                                                                                            // Apertura della finestra di gestione degli Item
            FXMLWindowLoader.loadFXML(getClass().getResource("/com/unifisweproject/hotelsupplymanagement/supplier/SupplierManagementWindow.fxml"),
                    view, true, event, "Gestione fornitori", false);
        }
        catch (IOException e) {
    ...
    public void handleActionEvent(ActionEvent event) {
        ...
        if (event.getSource() instanceof Button button) {
            switch (button.getId()) {
                ...
                case "modifyButton" -> {
                    Supplier selectedSupplier = view.getSupplierTable().getSelectionModel().getSelectedItem();
